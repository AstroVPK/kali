...
parser = argparse.ArgumentParser(description = "Compute the log-likelihood of a lightcurve specified by configuration file 'cf' and 'old' in working directory 'pwd' using C-ARMA parameter values ")
	...
	if args.p:
		if args.q:
			if len(args.theta) != int(args.p) + int(args.q) + 1:
				...
				computeLnLikeTask(args.pwd, args.cf, Stamp, *args.theta, p = args.p, q = args.q).run()
		else:
			print "Must supply q!"
			sys.exit(1)
	else:
