...
int main() {
	cout.clear();
	cout << endl;
	cout << "Program: recoveryTest" << endl;
	cout << "Purpose: Program to test C-ARMA modelling (recovery test)." << endl;
	...
	int nwalkers = 2*(pMaster + qMaster + 1), nsteps = 0;
	do {
		AcquireInput(cout,cin,"Number of walkers to use: ","Invalid value.\n",nwalkers);
		} while (nwalkers < 2*(pMaster + qMaster + 1));
	do {
	...
	for (int tNum = 0; tNum < nthreads; tNum++) {
		Systems[tNum].allocCARMA(pMaster,qMaster);
	...
	opt opt(nlopt::LN_NELDERMEAD, pMaster + qMaster + 1);
