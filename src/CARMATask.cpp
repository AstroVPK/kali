...
kali::CARMATask::CARMATask(int pGiven, int qGiven, int numThreadsGiven, int numBurnGiven) {
	...
	ThetaVec = static_cast<double*>(_mm_malloc(numThreads*(kali::CARMATask::r + p + q + 1)*sizeof(double),64));
	for (int threadNum = 0; threadNum < numThreads; ++threadNum) {
		Systems[threadNum].allocCARMA(p,q);
		setSystemsVec[threadNum] = false;
		#pragma omp simd
		for (int i = 0; i < (kali::CARMATask::r + p + q + 1); ++i) {
			ThetaVec[i + threadNum*(kali::CARMATask::r + p + q  +1)] = 0.0;
			}
		}
	}
...
void kali::CARMATask::get_Theta(double *Theta, int threadNum) {
	for (int i = 0; i < (kali::CARMATask::r + p + q + 1); ++i) {
		Theta[i] = ThetaVec[i + threadNum*(kali::CARMATask::r + p + q + 1)];
		}
	}
...
int kali::CARMATask::set_System(double dt, double *Theta, int threadNum) {
	...
	if (setSystemsVec[threadNum] == true) {
		...
		for (int i = 0; i < (kali::CARMATask::r + p + q + 1); ++i) {
			if (ThetaVec[i + threadNum*(kali::CARMATask::r + p + q + 1)] != Theta[i]) {
				alreadySet = false;
				}
			}
		} else {
	...
	if (alreadySet == false) {
		...
		if (goodYN == 1) {
			for (int i = 0; i < (p + q + 1); ++i) {
				ThetaVec[i + threadNum*(kali::CARMATask::r + p + q + 1)] = Theta[i];
				}
