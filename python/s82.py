...
class sdssLC(libcarma.basicLC):
	...
	def plot(self, fig = -1, doShow = False, clearFig = True):
		...
		plt.xlim(self.t[0], self.t[-1])
		if self.isSmoothed:
			plt.plot(self.tSmooth, self.xSmooth, color = self.smoothColorDict[self.band], marker = 'o', markeredgecolor = 'none', zorder = -5)
			plt.plot(self.tSmooth, self.xSmooth, color = self.smoothColorDict[self.band], zorder = -5)
			plt.fill_between(self.tSmooth, self.xSmooth - self.xerrSmooth, self.xSmooth + self.xerrSmooth, facecolor = self.smoothErrColorDict[self.band], alpha = 0.5, zorder = -5)
